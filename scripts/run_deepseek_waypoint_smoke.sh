...
/file_system/vepfs/algorithm/intern03/.conda/envs/lhvln/bin/python \
  ...
  --max-decisions "${MAX_DECISIONS:-25}" \
  --max-candidates "${MAX_CANDIDATES:-6}" \
  --render \
  --time-prompt-mode dynamic_fuzzy \
  --navgpt-command "/file_system/vepfs/algorithm/intern03/.conda/envs/lhvln/bin/python tools/deepseek_waypoint_selector.py --model ${DEEPSEEK_MODEL:-deepseek-chat}" \
  --output-dir "${OUTPUT_DIR:-output/time_aware_scene/navgpt_waypoint_deepseek_smoke}" \
  --summary-csv "${SUMMARY_CSV:-output/time_aware_scene/navgpt_waypoint_deepseek_smoke/summary.csv}" \
  --save-prompts \
  --quiet
